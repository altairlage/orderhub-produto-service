...
public class ProdutoApiControllerTest {
    ...
    void testBuscarProdutoPorId_Sucesso() {
        ProdutoDTO dto = new ProdutoDTO(1L, "Produto", "Desc", 10.0, 5);
        ...
        assertEquals("Produto", response.getBody().getNome());
    }
    ...
    void testBuscarProdutoPorNome() {
        ProdutoDTO dto = new ProdutoDTO(2L, "Nome", "Info", 15.0, 1);
    ...
    void testCriarProduto() {
        ...
        ProdutoDTO dto = new ProdutoDTO(3L, "Novo", "Item", 50.0, 0);
    ...
    void testAtualizarProduto() {
        ...
        ProdutoDTO dto = new ProdutoDTO(5L, "Atual", "Item", 77.0, 0);
        when(produtoController.atualizarProduto(any())).thenReturn(dto);

        var response = produtoApiController.atualizarProduto(5L, request);
        assertEquals("Atual", response.getBody().getNome());
    }

    @Test
    void testAtualizarProduto_NotFound() {
        ProdutoApiRequestDto request = new ProdutoApiRequestDto("Nada", "Item", 11.0);
        when(produtoController.atualizarProduto(any())).thenThrow(new ProdutoNaoEncontradoException(""));

        var response = produtoApiController.atualizarProduto(6L, request);
        assertEquals(404, response.getStatusCodeValue());
    }
}
